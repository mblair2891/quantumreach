...
## Post-authentication entry
Generic authenticated entry routes portal-only `CLIENT` members to `/portal`, subscriber workspace members to `/dashboard`, and `ADMIN_EMAILS` platform operators to `/platform`. Onboarding continues to take precedence when no active workspace membership exists; authorized explicit deep links are preserved.
