...
Workspace administrators configure allowed models, feature defaults, project restrictions, client IDs, and monthly alerts as structured settings. Client portal users can only select enabled providers/models permitted for their user and project. Selection records subscriber-paid usage; clients cannot view, add, rotate, test, or remove credentials. Manual fallback remains available.
