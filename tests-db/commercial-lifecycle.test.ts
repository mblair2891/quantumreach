...
it("repairs a partially initialized setup catalog idempotently without erasing mappings", async () => {
  ...
  const keys = ["STANDARD_SETUP", "PRIORITY_SETUP"];
  const catalogKeys = [...keys, "EXPEDITED_SETUP"];
  const rollback = new Error("ROLLBACK_SETUP_CATALOG_TEST");
  // Shared catalog keys can be referenced by leftover SaasSubscriptionItem rows
  // from other suites. Clear RESTRICT FKs inside the rolled-back transaction only.
  await expect(db.$transaction(async (tx) => {
    const existing = await tx.commerceProduct.findMany({ where: { key: { in: catalogKeys } }, select: { id: true } });
    const productIds = existing.map((row) => row.id);
    if (productIds.length) {
      await tx.saasSubscriptionItem.deleteMany({ where: { commerceProductId: { in: productIds } } });
    }
    ...
    await tx.commerceProduct.create({
      data: {
        ...
        billingInterval: "ONE_TIME",
        sortOrder: 99,
        stripeProductId: "prod_test_preserve",
        stripePriceId: "price_test_preserve",
        metadata: { priceCents: 32100, operatorNote: "keep" },
      },
    });
    await upsertSetupProducts(tx as any);
    await upsertSetupProducts(tx as any);
    const products = await tx.commerceProduct.findMany({
      where: { key: { in: keys }, category: "SETUP_FEE", active: true },
    ...
    expect(await tx.commerceProduct.findUnique({ where: { key: "EXPEDITED_SETUP" } })).toBeNull();
    expect(products.find((p) => p.key === "PRIORITY_SETUP")).toMatchObject({
      stripeProductId: "prod_test_preserve",
      stripePriceId: "price_test_preserve",
      metadata: expect.objectContaining({ priceCents: 32100, operatorNote: "keep", setupPriorityProduct: true }),
    });
    throw rollback;
