...
export function resolveSubscriberLifecycle(facts: LifecycleFacts): SubscriberLifecycleSnapshot {
  ...
  if (!facts.profileComplete) return snapshot("JOIN_PROFILE_REQUIRED", completed, "Subscriber profile is incomplete.", false, "Complete your profile", "/join");
  ...
  if (facts.order.status === "FAILED" || facts.infrastructure.status === "FAILED") return snapshot("FAILED", completed, "Provisioning needs operator review.", true, "Retry or contact support", "/setup/status");
  if (facts.order.paymentStatus !== "PAID") return snapshot("AWAITING_FINANCIAL_CLEARANCE", completed, "Authorized financial clearance is pending.", false, "Wait for operator clearance", "/setup/status");
  completed.push("financial-clearance");
  if (!(facts.workspaceReady && facts.membershipReady && facts.subscriptionReady && facts.entitlementsReady)) return snapshot("PROVISIONING", completed, "Workspace activation is still running.", true, "Refresh setup status", "/setup/status");
  completed.push("workspace", "membership", "subscription", "entitlements");
  if (!facts.onboardingComplete) return snapshot("ONBOARDING_REQUIRED", completed, null, false, "Complete subscriber onboarding", "/dashboard/onboarding", true);
