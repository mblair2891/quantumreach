...
export async function processStripeEvent(event: StripeEvent) {
  ...
  try {
    if ((event.type === "checkout.session.completed" && event.data.object.payment_status === "paid") || event.type === "checkout.session.async_payment_succeeded") await syncPaid(event.data.object, event.id);
    if (["checkout.session.async_payment_failed", "payment_intent.payment_failed"].includes(event.type)) { const order=await findOrder(event.data.object); if(order && order.paymentStatus!=="PAID") await prisma.customerOrder.update({where:{id:order.id},data:{paymentStatus:"FAILED",status:"FAILED"}}); }
    ...
    const order=await findOrder(event.data.object); await prisma.stripeWebhookEvent.update({ where: { id: record.id }, data: { status: "PROCESSED", processedAt: new Date(), safeError: null, customerOrderId: order?.id } });
  } catch (error) {
