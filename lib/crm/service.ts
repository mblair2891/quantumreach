import { enforceAcceptedLimit } from "@/lib/commercial/limits";
...
export async function getLeadDetail(workspaceId: string, id: string) { await requireWorkspaceAccess(workspaceId); const record = await prisma.lead.findFirst({ where: { id, workspaceId }, include: { outreachStatuses: { include: { campaign: true }, orderBy: { updatedAt: "desc" } }, callSessions: { orderBy: { updatedAt: "desc" } } } }); if (!record) notFound(); return record; }
