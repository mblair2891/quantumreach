...
export async function runAnalyzer(workspaceId: string, analyzerKey: keyof typeof analyzerPrompts, input: Record<string, unknown>) {
  ...
  const run = await prisma.analyzerRun.create({ data: { workspaceId, analyzerDefinitionId: definition.id, input, status: "RUNNING", startedAt: new Date(), createdById: user.id } });
  const execution = await prisma.aIExecution.create({ data: { workspaceId, analyzerRunId: run.id, provider: "openai", model: process.env.OPENAI_DEFAULT_MODEL ?? "gpt-4.1-mini", input, status: "RUNNING", startedAt: new Date(), createdById: user.id } });
  try {
    ...
    const [updatedRun] = await prisma.$transaction([
      prisma.analyzerRun.update({ where: { id: run.id }, data: { output: result.json as object, status: "SUCCEEDED", completedAt: new Date(), reviewStatus: "NEEDS_REVIEW" } }),
      prisma.aIExecution.update({ where: { id: execution.id }, data: { output: result.json as object, status: "SUCCEEDED", model: result.model, completedAt: new Date() } }),
      prisma.aIOutputArtifact.create({ data: { workspaceId, analyzerRunId: run.id, aiExecutionId: execution.id, artifactType: analyzerKey, content: result.json as object, reviewStatus: "DRAFT", createdById: user.id } })
    ]);
