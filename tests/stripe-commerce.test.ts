...
const createCustomer = vi.fn(); const listCustomers = vi.fn(); const updateCustomer = vi.fn(); const createSession = vi.fn(); const retrieveSession = vi.fn(); const createPortalSession = vi.fn(); const constructEvent = vi.fn(); const fulfill = vi.fn(); const issueSetup = vi.fn();
const stripeClient = {
...
vi.mock("@/lib/auth/account-setup", () => ({ issueAccountSetupToken: issueSetup }));
