...
export async function NewMeetingPage() {
  ...
  return <div className="space-y-6"><div><Link href="/dashboard/meetings" className="text-sm text-slate-500 hover:underline">← Back to meetings</Link><h1 className="mt-2 text-3xl font-semibold">New meeting</h1><p className="mt-2 text-slate-600 dark:text-slate-300">Zoom meetings are created in the connected workspace Zoom account when available; legacy native meetings remain optional.</p></div>
    <Card><CardHeader><CardTitle>Meeting details</CardTitle><CardDescription>All linked records are validated against the active workspace.</CardDescription></CardHeader><form action={create} className="grid gap-4 md:grid-cols-2">
      ...
      <label className="grid gap-1 text-sm font-medium">Provider<Select name="provider" defaultValue={options.zoomConnected ? "ZOOM" : "NATIVE_LIVEKIT"}>{options.zoomConnected ? <option value="ZOOM">Zoom</option> : null}<option value="NATIVE_LIVEKIT">Native Quantum Reach (Legacy/Optional)</option><option value="EXTERNAL_LINK">External link</option></Select></label>
      ...
      <fieldset className="grid gap-3 rounded-2xl border p-4 md:col-span-2 dark:border-slate-800"><legend className="px-1 text-sm font-semibold">Meeting settings</legend><label className="flex gap-3 text-sm"><input type="checkbox" name="recordingPlanned" /> <span><span className="font-medium">Record this meeting</span><span className="block text-slate-500">Prepare this meeting for provider recording and Quantum Reach transcript review.</span></span></label><label className="flex gap-3 text-sm"><input type="checkbox" name="recordingConsentRequired" checked readOnly /> <span><span className="font-medium">Require recording consent before entry</span><span className="block text-slate-500">Participants must consent before entering a recorded meeting.</span></span></label><label className="flex gap-3 text-sm"><input type="checkbox" name="lobbyEnabled" /> <span><span className="font-medium">Use provider waiting room / legacy lobby (Keep participants in lobby until host admits them)</span><span className="block text-slate-500">For Zoom meetings, Zoom waiting room is the runtime authority; legacy native meetings use Quantum Reach lobby controls.</span></span></label></fieldset>
...
export async function MeetingDetailPage({ id, invitationUrl }: { id: string; invitationUrl?: string }) {
  ...
  return <div className="space-y-6"><div><Link href="/dashboard/meetings" className="text-sm text-slate-500 hover:underline">← Back to meetings</Link><div className="mt-2 flex flex-wrap items-center gap-3"><h1 className="text-3xl font-semibold">{meeting.title}</h1><span className="rounded-full border px-3 py-1 text-xs font-medium">{label(meeting.status)}</span></div><p className="mt-2 text-slate-600 dark:text-slate-300">{meeting.description || "No description"}</p></div>
    ...
    <Card><CardHeader><CardTitle>Edit meeting</CardTitle><CardDescription>Linked CRM and CallSession records remain workspace-scoped.</CardDescription></CardHeader><form action={update} className="grid gap-4 md:grid-cols-2">
      ...
      <fieldset className="grid gap-3 rounded-2xl border p-4 md:col-span-2 dark:border-slate-800"><legend className="px-1 text-sm font-semibold">Meeting settings</legend><label className="flex gap-3 text-sm"><input type="checkbox" name="recordingPlanned" defaultChecked={meeting.recordingPlanned} /> <span><span className="font-medium">Record this meeting</span><span className="block text-slate-500">Prepare this meeting for provider recording and Quantum Reach transcript review.</span></span></label><label className="flex gap-3 text-sm"><input type="checkbox" name="recordingConsentRequired" checked readOnly /> <span><span className="font-medium">Require recording consent before entry</span><span className="block text-slate-500">Participants must consent before entering a recorded meeting.</span></span></label><label className="flex gap-3 text-sm"><input type="checkbox" name="lobbyEnabled" defaultChecked={meeting.lobbyEnabled} /> <span><span className="font-medium">Use provider waiting room / legacy lobby (Keep participants in lobby until host admits them)</span><span className="block text-slate-500">For Zoom meetings, Zoom waiting room is the runtime authority; legacy native meetings use Quantum Reach lobby controls.</span></span></label></fieldset>
