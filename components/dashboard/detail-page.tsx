...
import { analysisFormDefaults, analysisProposalOpportunityId, generateExecutiveReport, generateProposal, generateStrategicRoadmap, getAnalysisDetail, getProposalDetail, getReportDetail, getRoadmapDetail, proposalDetailPath, readablePhases, readableSections, setAnalysisStatus, setDeliverableStatus, updateAnalysis, updateBusinessCase, updateProposal } from "@/lib/reports/service";
...
function SourcesUsed({ sources }: { sources?: Array<{ documentId: string; title: string; version: string; documentType: string; workflowStage: string; chunkIds: string[]; chunkIndexes: number[] }> }) {
  return <Card><CardHeader><CardTitle>Sources used</CardTitle><CardDescription>Governed source-of-truth excerpts recorded for this AI-generated output.</CardDescription></CardHeader><CardContent className="space-y-2 text-sm">{sources?.length ? sources.map((source) => <div key={`${source.documentId}-${source.workflowStage}`} className="rounded-md border border-slate-200 p-3 dark:border-slate-800"><p className="font-medium">{source.title} <span className="text-xs text-slate-500">v{source.version}</span></p><p className="text-slate-600 dark:text-slate-300">{source.documentType} · {source.workflowStage} · {source.chunkIds.length} chunks</p></div>) : <p className="text-slate-500">No active source-of-truth usage was recorded. Generation may have used base defaults.</p>}</CardContent></Card>;
}
...
export async function AnalysisDetailPage({ id }: { id: string }) {
  ...
  const { analysis, events, sources } = await getAnalysisDetail(wid, id);
  ...
  async function proposal(formData: FormData) { "use server"; const wid = await workspaceId(); const created = await generateProposal(wid, String(formData.get("opportunityId") ?? ""), id); redirect(proposalDetailPath(created)); }
