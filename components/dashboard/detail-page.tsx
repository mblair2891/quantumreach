...
export async function AnalysisDetailPage({ id }: { id: string }) {
  ...
  async function mark(formData: FormData) { "use server"; const wid = await workspaceId(); await setAnalysisStatus(wid, id, formData.get("status")); revalidatePath(`/dashboard/analysis/${id}`); }
