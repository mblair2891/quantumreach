...
export function Button({ className, variant = "default", asChild, children, ...props }: Props) {
  const classes = cn("inline-flex items-center justify-center rounded-xl px-4 py-2 text-sm font-medium transition focus:outline-none focus:ring-2 focus:ring-ring disabled:opacity-50", variant === "default" && "bg-primary text-primary-foreground hover:bg-slate-800", variant === "outline" && "border bg-white hover:bg-slate-50", variant === "ghost" && "hover:bg-slate-100", className);
