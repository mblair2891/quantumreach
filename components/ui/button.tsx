...
export function Button({ className, variant = "default", asChild, children, ...props }: Props) {
  const classes = cn("inline-flex min-h-10 items-center justify-center rounded-xl px-4 py-2 text-sm font-medium leading-5 transition focus:outline-none focus:ring-2 focus:ring-ring disabled:opacity-50", variant === "default" && "bg-primary text-primary-foreground hover:bg-slate-800 dark:hover:bg-slate-200", variant === "outline" && "border border-border bg-card hover:bg-slate-50 dark:hover:bg-slate-800", variant === "ghost" && "hover:bg-slate-100 dark:hover:bg-slate-800", className);
