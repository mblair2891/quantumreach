...
export async function assignProductAction(form: FormData) { const ctx = await requireOperatorAccess(); await assignWorkspaceCommerceProduct({ workspaceId: String(form.get("workspaceId")), productId: String(form.get("productId")), quantity: Number(form.get("quantity")), source: String(form.get("source")) as any, startsAt: date(form.get("startsAt")), endsAt: date(form.get("endsAt")), note: String(form.get("note") || "") || null, assignedById: ctx.user.id }); revalidatePath("/platform/subscriptions"); revalidatePath(`/platform/workspaces/${String(form.get("workspaceId"))}`); revalidatePath("/dashboard/sending"); }
