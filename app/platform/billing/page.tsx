...
export default async function PlatformBillingPage() {
  const [orders, subscriptions, events] = await Promise.all([
    prisma.customerOrder.findMany({ orderBy: { updatedAt: "desc" }, take: 100, include: { infrastructureOrder: true } }),
  ...
  return <main className="space-y-8"><header><h1>Stripe billing operations</h1><p>Provider identifiers, clearance state, refunds, disputes, subscriptions, and safely recorded webhook failures.</p></header>
    <section className="overflow-x-auto rounded-xl border"><table className="w-full text-sm"><thead><tr><th>Order</th><th>Provider / clearance</th><th>Stripe references</th><th>Payment</th><th>Refund / dispute</th><th>Setup</th></tr></thead><tbody>{orders.map(order => { const payment = paymentByOrder.get(order.id); const dispute = disputeByOrder.get(order.id); return <tr key={order.id} className="border-t align-top"><td>{short(order.id)}<br /><span className="text-slate-500">{short(order.userId)}</span></td><td>{order.paymentMethod}<br />{order.paymentStatus}</td><td>Customer {short(order.stripeCustomerId)}<br />Checkout {short(order.stripeCheckoutSessionId)}<br />Payment {short(order.stripePaymentIntentId)}<br />Subscription {short(order.stripeSubscriptionId)}</td><td>{payment?.paidAt?.toLocaleString() ?? "No verified payment"}</td><td>{order.refundedAmountCents ? `${order.refundedAmountCents} refunded` : "No refund"}<br />{dispute ? `Dispute: ${dispute.status}` : "No active dispute"}</td><td>{order.infrastructureOrder?.status ?? "No setup order"}</td></tr>; })}</tbody></table>{!orders.length && <p className="p-5">No customer orders.</p>}</section>
