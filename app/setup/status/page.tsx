...
export default async function SetupStatus({ searchParams }: { searchParams?: { testPayment?: string } }) {
  ...
  return <FunnelShell><main className="mx-auto max-w-3xl px-5 py-12 sm:px-8 sm:py-16"><p className="funnel-eyebrow">Subscriber lifecycle</p><h1 className="funnel-title mt-3 text-4xl font-semibold">Your setup status</h1>{isSimulatedPaymentEnvironment() && setup?.order.paymentMethod === "SIMULATED_TEST" ? <p className="mt-5 rounded-xl border border-indigo-300 bg-indigo-50 p-4 text-sm font-semibold text-indigo-950 dark:border-indigo-700 dark:bg-indigo-950/40 dark:text-indigo-100">{searchParams?.testPayment === "completed" ? "Test payment completed" : "Test payment"} — no real charge was made.</p> : null}<p className="funnel-copy mt-3">Current stage: <strong>{lifecycle.stage.replaceAll("_", " ")}</strong></p><div className="mt-8 overflow-hidden rounded-2xl border bg-white">{rows.map(([label, value]) => <div className="flex justify-between gap-5 border-b px-5 py-4 last:border-0" key={label}><span className="text-slate-700">{label}</span><strong className="text-right">{value}</strong></div>)}</div>{lifecycle.blockingCondition && <p className="mt-6 rounded-xl bg-amber-50 p-4 text-amber-900">{lifecycle.blockingCondition}</p>}<section className="mt-6 funnel-card p-6"><h2 className="font-semibold">Next action</h2><p className="mt-2 text-slate-700">{lifecycle.nextAction}</p><div className="mt-5 flex flex-wrap gap-3"><Link className="funnel-primary" href={lifecycle.route}>{lifecycle.dashboardReady ? "Continue" : "Check next step"}</Link><Link className="funnel-secondary" href="/setup/status">Refresh status</Link><a className="funnel-secondary" href="mailto:support@quantumreach.ai">Contact support</a></div></section><p className="mt-5 text-sm text-slate-600">Managed infrastructure is tracked independently and may remain deferred without blocking an otherwise active dashboard.</p></main></FunnelShell>;
}
