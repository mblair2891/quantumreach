...
export default async function Page({ params, searchParams }: { params: { slug: string }; searchParams: { token?: string; invite?: string } }) {
  const { meeting, accessError } = await getPublicMeeting(params.slug, searchParams.invite ?? searchParams.token);
  ...
  return <MeetingClient meeting={{ id: meeting.id, title: meeting.title, description: meeting.description, scheduledAt: meeting.scheduledAt?.toISOString() }} invitationToken={searchParams.invite ?? searchParams.token} defaultDisplayName={meeting.invitationDisplayName ?? undefined} />;
}
